...
%Sync_Send_2(nr_kanalu(1-6), pozycje katowe w³¹cz-wy³¹cz) - sterowanie blokad¹ generatorów
%Sync_Send_2(10,[a b])  - sterowanie natryskiwaniem, piêæ ostatnich bitów zmiennej a odpowiada zadanym stanom natryskiwaczy
%Sync_Send_2(11, [0 0]) - odczyt licznika liczby obrotów taœmy
...
function pos = Sync_Send_2(command, arg)
    ...
    if (isa(command, 'double'))
        assert(nargin == 2, 'SYNC: NiewÅ‚aÅ›ciwa iloÅ›Ä‡ parametrÃ³w');
    ...
    if sync_obj.conn.BytesAvailable > 1
        ...
        fprintf('BÅ‚Ä…d (1): %d bajtow (%s)\n', length(resp), resp);
    else
        resp = fread(sync_obj.conn, 1);
        if (resp ~= '@')
            fprintf('BÅ‚Ä…d (2): znak %d (%s)\n', resp, resp);
        else
            fprintf('OK\n');
        end;
    end;
...
function msg = Sync_CreateMessage(channel, points)
    ...
    if CH == 10
        ...
        % Przekszta³æ czas w sekundach na jednostê wewnêtrzn¹ (10ms):
